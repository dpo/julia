...
template<size_t n>
using FeatureList = std::array<uint32_t,n>;
                                                         ...
                                                         uint32_t nfeature)
{
    ...
    std::vector<uint8_t> res(8 + 4 * nfeature + namelen);
    memcpy(&res[0], &nfeature, 4);
    memcpy(&res[4], features, 4 * nfeature);
    memcpy(&res[4 + 4 * nfeature], &namelen, 4);
    memcpy(&res[8 + 4 * nfeature], name, namelen);
    return res;
}
...
{
    return serialize_target_data(name, &features[0], n);
}
...
template<typename CPU, size_t n>
static inline auto deserialize_target_data(const CPUSpec<CPU,n> *cpus,
                                           size_t ncpu, const uint8_t *data,
                                           uint32_t def=0) -> std::vector<TargetData<CPU,n>>
{
    uint32_t ntarget;
    memcpy(&ntarget, data, 4);
    data += 4;
    std::vector<TargetData<CPU,n>> res(ntarget);
    for (uint32_t i = 0; i < ntarget; i++) {
        ...
        if (nfeature < n) {
            ...
            memcpy(&target.features[0], data, 4 * n);
        }
        data += 4 * nfeature;
        uint32_t namelen;
        ...
        target.cpu = find_cpu_id(target.name.c_str(), cpus, ncpu, def);
    }
    return res;
}
...
constexpr CPUSpec<CPU, feature_sz> cpus[] = {
...
{
    using namespace X86;
    return jl_cstr_to_string(find_cpu_name(host_cpu.first, cpus, sizeof(cpus) / sizeof(cpus[0])));
}
...
jl_sysimg_fptrs_t jl_init_processor_sysimg(void *hdl)
{
    ...
    return parse_sysimg(hdl, [] (void *id) {
            auto targets = deserialize_target_data(cpus, sizeof(cpus) / sizeof(cpus[0]),
                                                   (const uint8_t *)id);
            // TODO
            return 0;
        });
